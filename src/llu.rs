...
pub static mut ARR: [u8; 3] = [0u8; 3]; //// filling the array with 3 zeros
...
pub async fn unsafer(){
    ...
    *y = 2; // since y is a reference in order to change its value we have to dereference it first
    ...
    /*
        ...
        in rust if a type is being used in other scopes and it doesn’t implement the Copy trait 
        then rust moves the type and takes the ownership of that type otherwise the type can be 
        passed into other scopes without loosing its ownership this can be happened for heap data 
        types which have no fixed sized at compile time, but if don’t want to lose their ownership 
        we can either clone or borrow them and pass them by reference either immutably or mutably 
        

        when a type is going to dropped at the end of a block or scope there it can't be a borrow 
